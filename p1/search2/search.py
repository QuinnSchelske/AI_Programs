...
def generalSearch(problem, fringe):
    start = Node(problem.getStartState(), 'None' , 'Stop', 0)
    closed = []
    fringe.push(start)
    while(1):
        if fringe.isEmpty():
        ...
        if node.position not in closed:
            ...
            succ = problem.getSuccessors(node.position)
            for childState in succ:
                childNode = Node(childState[0], node, childState[1], 0)
...
def uniformCostSearch(problem):
    ...
    cost = lambda node: problem.getCostOfActions(node.getActionList())
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    cost = lambda node: problem.getCostOfActions(node.getActionList())+heuristic(node.position, problem)
    return generalSearch(problem, util.PriorityQueueWithFunction(cost))
